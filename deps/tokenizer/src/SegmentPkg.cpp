...
void ChineseCharTaggerImpl::init() {
  ...
  */
  ...
  //# NOTE 0x22xx, 0x23xx is number symbol,
  {
    ...
    ptr = new u1[256];
    memset(ptr, 0, 256);
    for (u2 i = 0x2018; i <= 0x201f; i++) ptr[(u1)i] = 'w';
    for (u2 i = 0x2025; i <= 0x2027; i++) ptr[(u1)i] = 'w';
    for (u2 i = 0x2032; i <= 0x2037; i++) ptr[(u1)i] = 'w';
    index_map[0x20] = ptr;
  }
}
/**
...
SegmentPkg::~SegmentPkg() {
  if (m_buf) free((void*)m_buf);
  m_buf = NULL;
  if (m_tag) free((void*)m_tag);
  m_tag = NULL;
}
...
int SegmentPkg::tagData(const char* buf, u1* tag, int l, int offset) {
  int length = l;
  if (!length) length = (int)strlen(buf);
  /*
  Utf8_Iter iter8;
