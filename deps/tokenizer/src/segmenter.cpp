...
1, det is too small
*/
int Segmenter::toLowerCpy(const u1* src, u1* det, u2 det_size) { return 0; }
...
const u1* Segmenter::peekToken(u2& aLen, u2& aSymLen, u2 n) {
  ...
  while (*ptr && (ptr < m_buffer_end) && i < CHUNK_BUFFER_SIZE) {
    ...
    // check tagger
    int num = m_unidict->findHits((const char*)ptr, &rs[1], 1024 - 1,
                                  MAX_TOKEN_LENGTH);
    if (num) {
      if (rs[1].length == len)
...
void Segmenter::popKwToken(u2 len) { return; }
