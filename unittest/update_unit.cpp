...
class IndexUpdateTest : public ::testing::Test {
 public:
 ...
 public:
  ...
  static void SetUpTestCase() {
    ...
    index->DBParams().path = std::string("/tmp/unit_") + std::string("update");
...
DefaultIndexWrapper *IndexUpdateTest::index = nullptr;
DocumentID IndexUpdateTest::document_id = 1;

TEST_F(IndexUpdateTest, AddOneDocument) {
  for (int i = 0; i < 10; i++)
    documents.push_back(TestUtil::NewDocument(GetDocumentID(), "a", 1, 1, 2));

  bool ret =
      index->index_writer_->UpdateDocuments(table, documents, nullptr, nullptr);
  EXPECT_FALSE(ret);  // document not exist
  for (auto du : documents) {
    EXPECT_EQ(wwsearch::kDocumentNotExistStatus, du->Status().GetCode());
  }
}

TEST_F(IndexUpdateTest, AddTwiceDocument) {
  for (int i = 0; i < 10; i++)
    documents.push_back(TestUtil::NewDocument(GetDocumentID(), "a", 1, 1, 2));
